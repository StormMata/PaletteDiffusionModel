...
from .util.mask import (smooth_blob, bottom_mask_456, blob_mask, bottom_mask_half, bottom_mask_4, bottom_mask_10, bbox2mask, brush_stroke_mask, get_irregular_mask, random_bbox, random_cropping_bbox)
...
def tensor_transforms(tensors, data_bounds):
    ...
    if x.shape[0] == 1:
        ...
        assert x.shape[0] == 2, f"tensor_transforms assumes 2 channel data! Shape of x is {x.shape}"
        ...
        assert x.shape[0] == 4, f"tensor_transforms assumes 4 channel data! Shape of x is {x.shape}"

        # Rescale to [-1,1]
        umin, umax, vmin, vmax, hpdmin, hpdmax, dpymin, dpymax = data_bounds

        print("STORM DEBUGGING ON 4-CHANNEL DATA")
        print("min/max before scaling")
        print("u:",    x[0].min(), x[0].max())
        print("v:",    x[1].min(), x[1].max())
...
class TestingDataset(data.Dataset):
    ...
    def get_mask(self):
        ...
        if self.mask_mode == 'smooth_blob':
            ...
            mask = bottom_mask_456(self.image_size)
        elif self.mask_mode == 'bottom_10':
            mask = bottom_mask_10(self.image_size)
